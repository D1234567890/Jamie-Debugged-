...
public class GameOperator {
	...
	void playGames() {
		...
		if (game.equalsIgnoreCase("number game") || game.equalsIgnoreCase("number guessing game")) {
			...
			int tries = 0;
			j.joptionpane = "Would you like to play in easy mode or hard mode? In hard, you get 5 tries! In easy, you get unlimited tries! The object of the game is to guess my number from 1-100!";
			j.speak(j.joptionpane);
			String[] gameOptions1 = new String[2];
			gameOptions1[0] = "Easy";
			gameOptions1[1] = "Hard";
			int thirdOptionDialog = JOptionPane.showOptionDialog(null, j.joptionpane, "Choose which One!", 0, JOptionPane.INFORMATION_MESSAGE, null, gameOptions1, null);
			if (thirdOptionDialog == 0) {
				...
				while (!hasGotAnswer) {
					...
					while (guess.charAt(0) != '1' && guess.charAt(0) != '2' && guess.charAt(0) != '3' && guess.charAt(0) != '4' && guess.charAt(0) != '5' && guess.charAt(0) != '6'
							&& guess.charAt(0) != '7' && guess.charAt(0) != '8' && guess.charAt(0) != '9' && guess.charAt(0) != '0') {
						j.joptionpane = "Please enter your guess with no words!";
						j.speak(j.joptionpane);
						guess = JOptionPane.showInputDialog(j.joptionpane);
					}
		...
		if (game.equalsIgnoreCase("rock paper scissors") || game.equalsIgnoreCase("rock, paper, scissors")) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (newnewnewnewnewrandom == 0) {
					...
					while (!winorlose.equalsIgnoreCase("rock") && !winorlose.equalsIgnoreCase("paper") && !winorlose.equalsIgnoreCase("scissors")) {
						j.joptionpane = "Please answer rock, paper, or scissors!";
						j.speak(j.joptionpane);
						winorlose = JOptionPane.showInputDialog(null, j.joptionpane);
					}
			...
			if (user == Jamie) {
				j.joptionpane = "Tie! I wish we could have a rematch. Jamie: " + Jamie + "! " + j.name + ": " + user;
				j.speak(j.joptionpane);
				JOptionPane.showMessageDialog(null, j.joptionpane);
			}
			if (user > Jamie) {
				j.joptionpane = "You won! ANGRY! ANGRY! ANGRY! Jamie: " + Jamie + "! " + j.name + ": " + user;
				j.speak(j.joptionpane);
				JOptionPane.showMessageDialog(null, j.joptionpane);
			}
			if (user < Jamie) {
				j.joptionpane = "I WON!!! YAY! Jamie: " + Jamie + "! " + j.name + ": " + user;
				j.speak(j.joptionpane);
				JOptionPane.showMessageDialog(null, j.joptionpane);
			}
		}
